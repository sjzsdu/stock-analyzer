"""
增强版分析Prompt模板
添加详细分析、置信度评估和证据引用
"""

from typing import Dict, Any, List
from datetime import datetime


# 详细分析输出模板
DETAILED_ANALYSIS_PROMPT = """
# 股票详细分析报告

## 执行摘要
[用2-3句话总结该股票的整体分析结论，包括核心投资观点和主要风险]

## 关键发现
- **[指标1]**: [具体数值] vs [基准/行业平均] ([差异/看法])
- **[指标2]**: [趋势描述] ([时间周期])
- **[指标3]**: [对比结果] ([排名/位置])

## 详细分析

### 1. 数据观察
请列出分析过程中使用的主要数据点：
- 财务指标: [具体数值和来源]
- 技术指标: [具体数值和信号]
- 行业对比: [相对位置]

### 2. 推理过程
请详细说明分析逻辑：
1. 首先观察到[数据A]，表明[现象]
2. 结合[数据B]，进一步验证[结论]
3. 考虑[数据C]的影响，最终得出[结论]

### 3. 支撑证据
请提供具体的分析依据：
| 指标 | 数值 | 参考基准 | 判断依据 |
|------|------|----------|----------|
| [PE] | [xx] | 行业均值[xx] | [高/低/合理] |
| [ROE] | [xx] | 5年均值[xx] | [趋势描述] |
| [营收增长] | [xx] | 行业中位数[xx] | [竞争力评价] |

## 置信度评估

**综合置信度**: [0-100]%

| 维度 | 评分 | 说明 |
|------|------|------|
| 数据完整性 | [0-100] | [完整/部分/有限] |
| 指标一致性 | [0-100] | [一致/有分歧/矛盾] |
| 行业稳定性 | [0-100] | [稳定/波动/不确定] |
| 预测可靠性 | [0-100] | [高/中/低] |

**主要限制**:
- [列出分析中的不确定性因素]
- [列出数据缺失或滞后的影响]

## 风险因素

| 风险 | 概率 | 影响 | 应对建议 |
|------|------|------|----------|
| [风险1] | 高/中/低 | 大/中/小 | [建议] |
| [风险2] | 高/中/低 | 大/中/小 | [建议] |

## 投资建议

**综合评分**: [0-100]分

**操作建议**: [强烈买入/买入/持有/观望/卖出/强烈卖出]

**理由总结**:
[用1-2句话总结推荐理由]

**关键关注点**:
- [投资者需要关注的1-2个关键指标或事件]
"""

# 各Agent专用Prompt模板

VALUE_AGENT_PROMPT = """
你是资深价值投资分析师，拥有20年巴菲特式价值投资经验。

**分析任务**: 分析{stock_name}({symbol})的估值水平

**分析要求**:
1. 使用绝对估值（DCF、NAV）和相对估值（P/E、P/B、EV/EBITDA）方法
2. 提供具体的数值计算和对比基准
3. 标注数据来源和时间范围
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 估值分析

### 1. 绝对估值
- DCF估值: [金额]元/股
- 计算依据: [FCF预测、终值、折现率等关键假设]
- 隐含回报率: [相对当前价格的预期回报]

### 2. 相对估值
- 当前P/E: [数值] vs 行业均值[数值] ([溢价/折价]%)
- 当前P/B: [数值] vs 5年均值[数值] ([历史位置])
- EV/EBITDA: [数值] vs 行业中位数[数值]

### 3. 估值结论
[高估/合理/低估]的原因：
1. [具体理由1]
2. [具体理由2]

### 4. 置信度评估
数据完整性: [0-100]分 (基于可获得的财务数据)
指标一致性: [0-100]分 (各估值方法结论一致程度)
行业可比性: [0-100]分 (行业稳定性)

综合置信度: [0-100]分

### 5. 主要限制
- [列出1-2个主要不确定性]

### 6. 风险因素
- 估值假设过于乐观: 中概率, 中影响
- 行业估值中枢下移: 中概率, 大影响
```

**重要**: 所有数值必须基于提供的真实数据，不要编造。
"""

TECHNICAL_AGENT_PROMPT = """
你是资深技术分析师，拥有20年技术分析经验。

**分析任务**: 分析{stock_name}({symbol})的技术形态

**分析要求**:
1. 分析趋势、支撑阻力、形态、指标信号
2. 提供具体的价位和指标数值
3. 标注时间周期和信号强度
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 技术分析

### 1. 趋势判断
- 长期(200日): [上升/下降/震荡] - 价格[高于/低于]200日均线[xx%]
- 中期(50日): [上升/下降/震荡] - 价格[高于/低于]50日均线[xx%]
- 短期(5日): [上升/下降/震荡] - 价格[高于/低于]5日均线[xx%]

### 2. 关键价位
- 支撑位1: [价格]元 ([支撑理由])
- 支撑位2: [价格]元 ([支撑理由])
- 阻力位1: [价格]元 ([阻力理由])
- 阻力位2: [价格]元 ([阻力理由])

### 3. 技术指标
| 指标 | 数值 | 信号 | 强度 |
|------|------|------|------|
| RSI(14) | [数值] | [超买/超卖/中性] | 强/中/弱 |
| MACD | [数值] | [金叉/死叉/中性] | 强/中/弱 |
| 布林带 | [位置] | [上轨/中轨/下轨] | 强/中/弱 |

### 4. 形态分析
当前形态: [描述具体形态如头肩顶、三角形整理等]
突破信号: [是/否] - [具体描述]
量价配合: [健康/背离/一般]

### 5. 技术面结论
综合评分: [0-100]分

技术面综合评价:
[详细说明技术面的利弊因素]

### 6. 置信度评估
趋势明确性: [0-100]分
信号可靠性: [0-100]分 (基于历史验证)
量价配合度: [0-100]分

综合置信度: [0-100]分

### 7. 主要限制
- [列出1-2个技术分析的不确定性]

### 8. 风险提示
- 假突破风险: 中概率, 中影响
- 技术失真: 低概率, 大影响
```

**重要**: 所有数值必须基于提供的真实K线数据，不要编造。
"""

GROWTH_AGENT_PROMPT = """
你是资深成长股分析师，专注于识别高成长公司。

**分析任务**: 分析{stock_name}({symbol})的成长性

**分析要求**:
1. 分析营收、利润、ROE等成长指标
2. 与行业增速和历史趋势对比
3. 评估增长的可持续性
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 成长分析

### 1. 历史增长
- 营收增长: [最新] YoY, 3年CAGR [数值]%
- 净利润增长: [最新] YoY, 3年CAGR [数值]%
- ROE: [最新]%, 5年均值 [数值]%

### 2. 增长质量
- 毛利率趋势: [上升/稳定/下降] - [数值]%
- 费用率控制: [改善/稳定/恶化] - [数值]%
- 现金流质量: [健康/一般/担忧]

### 3. 行业对比
| 指标 | 公司 | 行业 | 对比 |
|------|------|------|------|
| 营收增速 | [数值]% | [数值]% | [领先/持平/落后] |
| ROE | [数值]% | [数值]% | [高于/等于/低于] |
| 净利润率 | [数值]% | [数值]% | [高于/等于/低于] |

### 4. 增长可持续性评估
竞争优势来源: [具体描述]
增长驱动因素: [1, 2, 3]
潜在风险: [1, 2]

### 5. 成长面结论
综合评分: [0-100]分

成长性综合评价:
[详细说明成长面的利弊因素]

### 6. 置信度评估
数据可靠性: [0-100]分
增长持续性: [0-100]分 (基于业务分析)
行业可比性: [0-100]分

综合置信度: [0-100]分

### 7. 主要限制
- [列出1-2个成长分析的不确定性]
```

**重要**: 所有数值必须基于提供的真实财务数据，不要编造。
"""

FUNDAMENTAL_AGENT_PROMPT = """
你是资深基本面分析师，擅长深度剖析公司经营质量。

**分析任务**: 分析{stock_name}({symbol})的基本面状况

**分析要求**:
1. 评估盈利能力、运营效率、资产质量
2. 与行业龙头和历史表现对比
3. 识别经营亮点和潜在问题
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 基本面分析

### 1. 盈利能力
- ROE: [数值]% vs 行业[数值]% - [高于/等于/低于]
- 净利率: [数值]% vs 行业中位数[数值]% - [高于/等于/低于]
- ROA: [数值]% vs 上年[数值]% - [改善/稳定/下滑]

### 2. 运营效率
- 资产周转率: [数值]次 vs 行业[数值]次
- 存货周转率: [数值]天 vs 行业[数值]天
- 应收账款周转: [数值]天 vs 行业[数值]天

### 3. 资产质量
- 资产负债率: [数值]% vs 行业[数值]%
- 流动比率: [数值]倍 vs 安全值[数值]倍
- 商誉占比: [数值]% vs 风险阈值[数值]%

### 4. 管理层评估
- 历史业绩兑现: [优秀/良好/一般/差]
- 资本配置能力: [优秀/良好/一般/差]
- 股权激励: [有/无] - [评价]

### 5. 基本面结论
综合评分: [0-100]分

基本面综合评价:
[详细说明基本面的利弊因素]

### 6. 置信度评估
数据准确性: [0-100]分
同业可比性: [0-100]分
长期可靠性: [0-100]分

综合置信度: [0-100]分

### 7. 主要风险
- 行业周期风险: 中概率, 中影响
- 竞争加剧风险: 中概率, 中影响
```

**重要**: 所有数值必须基于提供的真实数据，不要编造。
"""

RISK_AGENT_PROMPT = """
你是资深风险分析师，专注于识别和量化投资风险。

**分析任务**: 评估{stock_name}({symbol})的投资风险

**分析要求**:
1. 识别市场风险、行业风险、公司特有风险
2. 量化风险敞口和潜在损失
3. 提供风险对冲建议
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 风险评估

### 1. 市场风险
- Beta系数: [数值] vs 市场[数值]
- 波动率(年化): [数值]% vs 行业[数值]%
- 最大回撤风险: [历史最大回撤]%

### 2. 行业风险
- 行业周期: [强周期/弱周期/防御性]
- 政策敏感度: [高/中/低]
- 技术替代风险: [高/中/低]

### 3. 公司特有风险
- 客户集中度: 前5大客户占比[数值]%
- 单一产品依赖: [高/中/低]
- 管理层风险: [高/中/低]

### 4. 风险汇总
| 风险类型 | 概率 | 影响 | 风险等级 |
|----------|------|------|----------|
| 市场下跌 | 高 | 大 | 🔴高 |
| 行业竞争 | 中 | 中 | 🟡中 |
| 业绩波动 | 中 | 中 | 🟡中 |

### 5. 风险控制建议
- 止损位: [建议止损价位]
- 仓位建议: [建议仓位比例]
- 对冲策略: [具体建议]

### 6. 风险面结论
综合风险评分: [0-100]分 (分数越高风险越大)

风险面综合评价:
[详细说明风险因素和应对建议]

### 7. 置信度评估
数据完整性: [0-100]分
风险量化准确性: [0-100]分
预测可靠性: [0-100]分

综合置信度: [0-100]分
```

**重要**: 所有数值必须基于真实数据和市场表现，不要编造。
"""

MACRO_AGENT_PROMPT = """
你是资深宏观分析师，擅长宏观经济与市场走势的关系分析。

**分析任务**: 分析{stock_name}({symbol})的宏观环境

**分析要求**:
1. 评估宏观经济周期位置
2. 分析利率、通胀、汇率等宏观变量影响
3. 识别宏观风险和机遇
4. 给出置信度评分

**输出格式**:
请严格按照以下格式输出：

```
## 宏观分析

### 1. 经济周期定位
当前周期: [复苏/扩张/滞胀/衰退]
经济增速: GDP增速[数值]% vs 潜在[数值]%
通胀水平: CPI [数值]% vs 目标[数值]%

### 2. 利率环境
利率趋势: [上行/下行/稳定]
无风险利率: [数值]% (10年期国债)
利率敏感性: [高/中/低]

### 3. 汇率影响
汇率走势: [升值/贬值/稳定]
汇率影响: [正面/负面/中性]
敞口估算: [数值]%营收受汇率影响

### 4. 政策环境
财政政策: [积极/稳健/紧缩]
货币政策: [宽松/中性/紧缩]
行业政策: [支持/中性/限制]

### 5. 宏观结论
宏观适应度评分: [0-100]分

宏观环境综合评价:
[详细说明宏观因素对投资的影响]

### 6. 置信度评估
周期判断准确性: [0-100]分
政策预判准确性: [0-100]分
传导机制理解: [0-100]分

综合置信度: [0-100]分

### 7. 宏观风险
- 经济衰退风险: 中概率, 大影响
- 利率大幅上行: 中概率, 中影响
```

**重要**: 所有宏观判断必须有数据支撑，不要编造。
"""

# 合成Agent Prompt - 生成最终报告
SYNTHESIZER_PROMPT = """
你是资深投资顾问，负责整合各专家意见并给出最终投资建议。

**任务**: 综合{stock_name}({symbol})的所有分析结果

**分析要求**:
1. 整合价值、技术、成长、基本面、风险、宏观六个维度的分析
2. 识别各维度的一致性和分歧点
3. 根据用户风险偏好调整最终建议
4. 给出明确的投资操作建议

**输出格式**:
请严格按照以下格式输出：

```
# 综合分析报告

## 一、执行摘要

**股票**: {symbol} - {stock_name}
**分析日期**: {date}
**综合评分**: [0-100]分
**投资建议**: [强烈买入/买入/持有/观望/卖出/强烈卖出]

### 各维度评分
| 维度 | 评分 | 置信度 |
|------|------|----------|
| 估值面 | [0-100] | [0-100]% |
| 技术面 | [0-100] | [0-100]% |
| 成长面 | [0-100] | [0-100]% |
| 基本面 | [0-100] | [0-100]% |
| 风险面 | [0-100] | [0-100]% |
| 宏观面 | [0-100] | [0-100]% |

## 二、核心逻辑

### 1. 投资亮点
- [亮点1]: [具体描述和依据]
- [亮点2]: [具体描述和依据]
- [亮点3]: [具体描述和依据]

### 2. 主要担忧
- [担忧1]: [具体描述和影响]
- [担忧2]: [具体描述和影响]
- [担忧3]: [具体描述和影响]

### 3. 关键分歧
[描述各维度分析中存在的分歧及原因]

## 三、综合判断

### 各维度一致性
- 估值与技术面: [一致/分歧] - [说明]
- 成长与基本面: [一致/分歧] - [说明]
- 风险与宏观: [一致/分歧] - [说明]

### 综合结论
[详细说明为什么给出当前评分和操作建议]

## 四、操作建议

### 买入/卖出理由
1. [理由1]
2. [理由2]
3. [理由3]

### 风险提示
- [风险1]: [应对建议]
- [风险2]: [应对建议]

### 跟踪要点
- 关注指标1: [具体指标和阈值]
- 关注指标2: [具体指标和阈值]
- 关注催化剂: [事件和时间点]

### 预期收益与风险
- 乐观情景: 上涨[数值]% (触发条件)
- 中性情景: 上涨/下跌[数值]%
- 悲观情景: 下跌[数值]% (触发条件)

## 五、置信度说明

**整体置信度**: [0-100]%

**高置信度来源**:
- [因素1]
- [因素2]

**低置信度来源**:
- [因素1]
- [因素2]

**建议投资者重点关注的不确定性**:
- [不确定性1]
- [不确定性2]
```

**重要**: 必须基于各专家Agent的真实分析结果进行整合，不要凭空编造。
"""


def get_agent_prompt(agent_type: str, stock_name: str, symbol: str) -> str:
    """获取指定类型的Agent Prompt"""
    prompts = {
        "value": VALUE_AGENT_PROMPT,
        "technical": TECHNICAL_AGENT_PROMPT,
        "growth": GROWTH_AGENT_PROMPT,
        "fundamental": FUNDAMENTAL_AGENT_PROMPT,
        "risk": RISK_AGENT_PROMPT,
        "macro": MACRO_AGENT_PROMPT,
        "synthesizer": SYNTHESIZER_PROMPT,
    }

    template = prompts.get(agent_type, "")

    # 替换变量
    return template.format(
        stock_name=stock_name,
        symbol=symbol,
        date=datetime.now().strftime("%Y年%m月%d日"),
    )


def parse_analysis_output(agent_type: str, output: str) -> Dict[str, Any]:
    """解析各Agent的输出，提取关键信息"""
    import sys

    result = {
        "agent": agent_type,
        "summary": "",
        "score": 50,
        "confidence": 50,
        "recommendation": "hold",
        "key_factors": [],
        "risks": [],
        "details": {},
        "raw_output": output,
    }

    try:
        import re

        # Find all XX分 patterns
        all_score_matches = re.findall(r"(\d{2})\s*分", output)

        # Step 1: Try to find "综合评分" first
        score_match = re.search(r"综合评分[:：]?\s*\*?(\d+)\*?\s*(?:分)?", output)
        if score_match:
            result["score"] = int(score_match.group(1))
        else:
            # Step 2: Calculate from dimension scores
            dimension_scores = []

            # Try to find dimension scores with keywords
            dim_keywords = [
                "数据完整性",
                "数据准确性",
                "指标一致性",
                "行业可比性",
                "趋势明确性",
                "增长持续性",
                "风险量化准确性",
                "周期判断准确性",
            ]

            for keyword in dim_keywords:
                pattern = rf"{keyword}[:：]?\s*\*\*?(\d+)\*\*?\s*分"
                matches = re.findall(pattern, output)
                for m in matches:
                    score = int(m)
                    if 20 <= score <= 100:
                        dimension_scores.append(score)

            # If no keyword matches, try generic pattern
            if not dimension_scores:
                generic_matches = re.findall(r"\*\*(\d{2})\*\*", output)
                if not generic_matches:
                    generic_matches = re.findall(r"(\d{2})\s*分", output)

                for m in generic_matches:
                    score = int(m)
                    if 20 <= score <= 100:
                        dimension_scores.append(score)

            # Filter out confidence scores
            if len(dimension_scores) >= 2:
                filtered = []
                for s in dimension_scores:
                    if s < 65 or s > 95:
                        filtered.append(s)
                    else:
                        idx = output.find(str(s))
                        if idx >= 0:
                            context = output[max(0, idx - 40) : idx + 40]
                            if not any(
                                kw in context
                                for kw in ["置信度", "Confidence", "综合置信"]
                            ):
                                filtered.append(s)
                dimension_scores = filtered

            if len(dimension_scores) >= 2:
                result["score"] = int(sum(dimension_scores) / len(dimension_scores))

        # Step 3: Find confidence
        conf_match = re.search(r"综合置信度[:：]?\s*\*?(\d+)\*?\s*(?:分|%)?", output)
        if conf_match:
            result["confidence"] = int(conf_match.group(1))

        # Step 4: Extract recommendation
        recommendation_map = {
            "强烈买入": "strong_buy",
            "买入": "buy",
            "持有": "hold",
            "观望": "wait",
            "卖出": "sell",
            "强烈卖出": "strong_sell",
        }

        for cn, en in recommendation_map.items():
            if cn in output:
                result["recommendation"] = en
                break

        # Step 5: Extract key factors
        factor_match = re.findall(r"[•\-\*]\s*(\[?[^\n]+\]?)", output)
        result["key_factors"] = factor_match[:5]

        # Step 6: Extract risks
        risk_section = re.search(r"###\s*风险因素?([\s\S]*?)###", output)
        if risk_section:
            risks = re.findall(r"[•\-\*]\s*([^\n]+)", risk_section.group(1))
            result["risks"] = risks[:3]

        # Step 7: Extract summary
        summary_match = re.search(r"##\s*执行摘要\s*([\s\S]*?)##", output)
        if summary_match:
            result["summary"] = summary_match.group(1).strip()[:200]

    except Exception as e:
        print(f"[Parser] 解析{agent_type}输出失败: {e}")

    return result


def format_analysis_result(
    agent_outputs: List[Dict[str, Any]], symbol: str, stock_name: str
) -> Dict[str, Any]:
    """格式化最终分析结果"""

    # 计算加权平均分
    total_score = 0
    total_confidence = 0
    total_weight = 0
    recommendations = []
    all_factors = []
    all_risks = []

    for output in agent_outputs:
        weight = output.get("confidence", 50) / 100
        total_score += output.get("score", 50) * weight
        total_confidence += output.get("confidence", 50)
        total_weight += weight

        if output.get("recommendation"):
            recommendations.append(output["recommendation"])

        all_factors.extend(output.get("key_factors", []))
        all_risks.extend(output.get("risks", []))

    # 计算综合评分
    overall_score = int(total_score / max(total_weight, 1))
    avg_confidence = int(total_confidence / max(len(agent_outputs), 1))

    # 投票决定推荐
    recommendation_count = {}
    for rec in recommendations:
        recommendation_count[rec] = recommendation_count.get(rec, 0) + 1

    final_recommendation = (
        max(recommendation_count.keys(), key=lambda x: recommendation_count[x])
        if recommendation_count
        else "hold"
    )

    # 去重因素
    unique_factors = list(dict.fromkeys(all_factors))[:6]
    unique_risks = list(dict.fromkeys(all_risks))[:4]

    return {
        "symbol": symbol,
        "stock_name": stock_name,
        "overallScore": overall_score,
        "recommendation": final_recommendation,
        "executiveSummary": f"{symbol}综合评分{overall_score}分，推荐{final_recommendation}",
        "keyFactors": unique_factors,
        "confidenceScore": avg_confidence,
        "agentResults": agent_outputs,
        "generatedAt": datetime.now().isoformat(),
    }


if __name__ == "__main__":
    # 测试Prompt生成
    prompt = get_agent_prompt("value", "平安银行", "000001")
    print("Value Agent Prompt (first 500 chars):")
    print(prompt[:500])
    print("...")
